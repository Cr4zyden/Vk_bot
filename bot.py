...
vk_session = vk_api.VkApi(token="ddece079e742244b127d32c16e3b62d33e177503f2900d401433317508d26e9396d2fa6c7bcde1e7eabd7")
        ...
        if is_message(event):
            if is_play_game(event):
                ...
                if not igra.is_unused_city(city):
                    send_message(user1,"Такой город уже был, такой город уже был назван")
                    send_message(user2,'Ты выйграл')
                    kill_game(igra)                    
                do_step(igra,city)
                send_message(user2,f"Твой ход, ходишь на букву: {igra.last_char.lower()}.\nБыл назван город: {str(igra.used_cities[-1]).title()}")
        
def do_step(igra,city):
    for game in active_games:
